...
CREATE TABLE IF NOT EXISTS `new_schema1`.`Post` (
  ...
  `Attachments_idAttachments` INT NULL,
  ...
  INDEX `fk_Post_Attachments1_idx` (`Attachments_idAttachments` ASC),
  ...
  CONSTRAINT `fk_Post_UserProfile2`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Post_Attachments1`
    FOREIGN KEY (`Attachments_idAttachments`)
    REFERENCES `new_schema1`.`Attachments` (`idAttachments`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
